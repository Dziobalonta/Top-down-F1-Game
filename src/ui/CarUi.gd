...
func _ready() -> void:
	...
	best_lap_label.horizontal_alignment = LabelAlignment

func update_values(car: Car, lap_count: int, total_laps: int, lap_time: float, best_time: float) -> void:
	...
	best_lap_label.text = "Best: %.2fs" % best_time
